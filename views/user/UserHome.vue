<template>
  <v-card>
    <v-card-title>
      <ListHeader>
        <span slot="tittle">{{Messages.users}}</span>
...
export default class UserHome extends BaseController {
  home = true;

  redirect() {
    this.$router.push({
      path: this.home ? "/usuarios/agregar" : "/usuarios"
    });
  }

  @Watch("$route", { immediate: true, deep: true })
  WatchRouter(to: any, from: any) {
    this.home = this.$router.currentRoute.name == "users" ? true : false;
  }  
}
</script>
